...
```json
{
  ...
  "so2": { "vgas": 1.234, "vref": 1.2, "mv": 34.0 },
...
- The firmware now discards the first few ESP32 ADC conversions after each `Vgas`/`Vref` channel switch so those high-impedance outputs can settle before averaging.
